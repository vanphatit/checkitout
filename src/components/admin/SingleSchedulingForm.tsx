...
import { Save, MapPin, Info, Bus as BusIcon } from 'lucide-react';
...
interface SingleSchedulingFormProps {
    onSuccess: () => void;
}

export default function SingleSchedulingForm({ onSuccess }: SingleSchedulingFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const result = await schedulingService.createScheduling(createData);

            // Show success message
            toast.success('Tạo lịch trình thành công');

            // Show conflict warnings if any buses were excluded
            if (result.conflicts && result.conflicts.length > 0) {
                result.conflicts.forEach((conflict, index) => {
                    setTimeout(() => {
                        toast.warning(`Xe ${conflict.plateNo}: ${conflict.message}`, {
                            duration: 5000,
                        });
                    }, (index + 1) * 300);
                });
