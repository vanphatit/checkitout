...
import { Plus, Search, MapPin, Edit, Filter, Clock, Navigation, AlertTriangle } from "lucide-react";
...
export default function RoutesPage() {
    ...
    const filteredRoutes = routes.filter(route => {
        const matchesSearch = route.name.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
            route.description?.toLowerCase().includes(debouncedSearch.toLowerCase());

        if (!statusFilter) return matchesSearch;

        if (statusFilter === 'active') return matchesSearch && route.isActive && !route.isDeleted;
        if (statusFilter === 'inactive') return matchesSearch && !route.isActive && !route.isDeleted;
        if (statusFilter === 'deleted') return matchesSearch && route.isDeleted;
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-neutral-200">
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="name">Tên tuyến</option>
                                <option value="distance">Khoảng cách</option>
                                <option value="basePrice">Giá cơ bản</option>
                            </select>
                        </div>
                        <div>
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                {route.stationIds.some((s: Station) => !s.isActive) && (
                                                    <div className="text-xs text-orange-600 flex items-center gap-1">
                                                        ...
                                                        <span>Có trạm ngừng hoạt động</span>
                                                    </div>
                                                )}
                                            </div>
                                        )}
